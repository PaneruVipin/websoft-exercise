...
const SOCKET_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://websoft-exercise.onrender.com";
...
export const mapBackendMessageToFrontendHelper = (
  ...
  const safeParticipatingUsers = Array.isArray(participatingUsers)
    ? participatingUsers
    : [];
  ...
  if (
    backendMsg.sender &&
    typeof backendMsg.sender === "object" &&
    "_id" in backendMsg.sender
  ) {
  ...
  if (
    backendMsg.receiver &&
    typeof backendMsg.receiver === "object" &&
    "_id" in backendMsg.receiver
  ) {
  ...
  const senderName =
    sender?.fullname || (senderId === currentUserId ? "You" : "User");
  const senderAvatarCharacter =
    senderName && senderName.length > 0 ? senderName[0].toUpperCase() : "U";
  const senderAvatar =
    sender?.avatar ||
    `https://placehold.co/40x40.png?text=${senderAvatarCharacter}`;
...
export const mapApiUserToAppUser = (apiUser) => {
  ...
  return {
    ...
    avatar: `https://placehold.co/40x40.png?text=${
      fullname && fullname.length > 0 ? fullname[0].toUpperCase() : "U"
    }`,
...
const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (threadsError) {
      console.error(
        "AppContext: Error fetching message threads:",
      ...
      toast({
        ...
        description:
          threadsError.message || "Could not fetch recent conversations.",
        variant: "destructive",
      });
    }
  }, [threadsError, toast]);
  ...
  useEffect(() => {
    if (!currentApiUser || !token) {
      console.log(
        "AppContext: No user or token. Clearing users and ensuring socket is disconnected."
      );
      setUsersState([]);
      return;
    }
    ...
    if (threadsData) {
      console.log(
        "AppContext: Received threadsData. Processing for user:",
        currentApiUser.fullname,
        threadsData.threads.length,
        "threads."
      );

      const usersFromApiThreads = threadsData.threads
        .map((thread) => {
        ...
        .filter(
          (apiUser) =>
            apiUser && currentApiUser && apiUser.id !== currentApiUser.id
        );
      ...
      setUsersState((prevUsers) => {
        ...
        usersFromApiThreads.forEach((threadUserFromApi) => {
          const existingPrevUser = prevUsers.find(
            (u) => u.id === threadUserFromApi.id
          );
          if (existingPrevUser) {
            finalUsersMap.set(threadUserFromApi.id, {
              ...threadUserFromApi,
              status:
                existingPrevUser.status === "online"
                  ? "online"
                  : threadUserFromApi.status || existingPrevUser.status,
              isOnline:
                existingPrevUser.status === "online"
                  ? true
                  : threadUserFromApi.isOnline || existingPrevUser.isOnline,
              lastMessageContent:
                threadUserFromApi.lastMessageContent ||
                existingPrevUser.lastMessageContent,
              unreadCount:
                threadUserFromApi.unreadCount !== undefined
                  ? threadUserFromApi.unreadCount
                  : existingPrevUser.unreadCount,
            });
          } else {
        ...
        console.log(
          "AppContext: Users state updated from threadsData. Total users:",
          newUsersArray.length,
          newUsersArray.map((u) => ({
            id: u.id,
            name: u.fullname,
            status: u.status,
      ...
      console.log(
        "AppContext: No thread data (or query finished with no data) for user:",
        currentApiUser.fullname,
        ". Current users in context:",
  ...
  const updateUserStatus = useCallback((userId, status) => {
    ...
    setUsersState((prevUsers) =>
      prevUsers.map((u) =>
        u.id === userId ? { ...u, status, isOnline: status === "online" } : u
      )
    );
  }, []);
  ...
  const addUserIfNotExists = useCallback((newUser) => {
    setUsersState((prevUsers) => {
      ...
      if (existingUserIndex === -1) {
        console.log(
          "AppContext addUserIfNotExists: Adding new user:",
          newUser.fullname,
          "Status:",
          newUser.status
        );
        return [
          ...
          {
            ...newUser,
            lastMessageContent: newUser.lastMessageContent || undefined,
            unreadCount: newUser.unreadCount || 0,
          },
        ];
      } else {
        ...
        console.log(
          "AppContext addUserIfNotExists: User already exists. Context user:",
        ...
        updatedUsers[existingUserIndex] = {
          ...
          ...newUser,
          status:
            contextUser.status === "online"
              ? "online"
              : newUser.status || contextUser.status,
          isOnline:
            contextUser.status === "online"
              ? true
              : newUser.isOnline || contextUser.isOnline,
          lastMessageContent:
            newUser.lastMessageContent ||
            contextUser.lastMessageContent ||
            undefined,
          unreadCount:
            newUser.unreadCount !== undefined
              ? newUser.unreadCount
              : contextUser.unreadCount || 0,
        };
        console.log(
          "AppContext addUserIfNotExists: Updated user in context:",
          updatedUsers[existingUserIndex].fullname,
          "Resolved status:",
          updatedUsers[existingUserIndex].status
        );
        return updatedUsers;
      }
    });
  ...
  const clearUserUnreadCount = useCallback((userIdToClear) => {
    console.log(
      `AppContext: Clearing unread count for userId: ${userIdToClear}`
    );
    setUsersState((prevUsers) =>
      prevUsers.map((user) =>
  ...
  useEffect(() => {
    if (currentApiUser && token) {
      if (!socketRef.current) {
        ...
        newSocket.on("disconnect", (reason) => {
          console.log(
            "AppContext: Socket disconnected:",
            reason,
            "- Socket.io client will attempt auto-reconnection."
          );
        ...
        newSocket.on("connect_error", (error) => {
          console.error(
            "AppContext: Socket connection error:",
            error.message,
            error.data
          );
          toast({
        ...
        newSocket.on("user_status_change", ({ userId, isOnline }) => {
          console.log("AppContext Event: user_status_change received", {
            userId,
            isOnline,
          });
        ...
        newSocket.on("receive_message", (backendMessage) => {
          console.log(
            "AppContext Event: receive_message received",
            backendMessage
          );
          if (!currentApiUser) return;

          const newMessage = mapBackendMessageToFrontendHelper(
            backendMessage,
            usersRef.current,
            currentApiUser.id
          );
          ...
          setMessages((prevMessages) => {
            if (prevMessages.some((msg) => msg.id === newMessage.id)) {
              console.log(
                "AppContext: receive_message - message already exists in live feed, not adding:",
                newMessage.id
              );
              return prevMessages;
            }
            console.log(
              "AppContext: receive_message - adding new message to live feed:",
              newMessage.id,
          ...
          if (
            ...
            typeof backendMessage.sender === "object" &&
            "_id" in backendMessage.sender
          ) {
          ...
          const otherUserIdInChat =
            senderIdFromMessage === currentApiUser.id
              ? typeof backendMessage.receiver === "string"
                ? backendMessage.receiver
                : backendMessage.receiver?._id
              : senderIdFromMessage;

          if (otherUserIdInChat) {
            console.log(
              `AppContext: Invalidating threadMessages for other user: ${otherUserIdInChat} (due to received message)`
            );
            queryClient.invalidateQueries({
              queryKey: ["threadMessages", otherUserIdInChat],
            });
            setUsersState((prev) =>
              prev.map((u) =>
          ...
          console.log(
            `AppContext: Invalidating messageThreads for current user: ${currentApiUser.id} (due to received message)`
          );
          queryClient.invalidateQueries({
            queryKey: ["messageThreads", currentApiUser.id],
          });
        ...
        newSocket.on("message_sent", (data) => {
          console.log(
            "AppContext Event: message_sent confirmation, ID:",
            data.messageId,
            "Temp ID:",
            data.tempId
          );
          setMessages((prevMessages) =>
            prevMessages.map((msg) =>
              data.tempId && msg.id === data.tempId
                ? { ...msg, id: data.messageId, timestamp: msg.timestamp }
                : msg
            )
          );
        });
        ...
        newSocket.on("typing", ({ fromUserId }) => {
          ...
          if (
            ...
            const timeoutId = setTimeout(() => {
              console.log(
                `AppContext: Timeout for ${fromUser.fullname} typing. Removing.`
              );
              setTypingUsers((prevTyping) =>
                prevTyping.filter((name) => name !== fromUser.fullname)
              );
      ...
      if (socketRef.current) {
        console.log(
          "AppContext: No currentApiUser/token, ensuring existing socket is disconnected and nullified."
        );
    ...
    return () => {
      ...
      if (socketRef.current && (!currentApiUser || !token)) {
        console.log(
          "AppContext: Cleanup - disconnecting socket because user/token became null."
        );
        socketRef.current.disconnect();
        socketRef.current.removeAllListeners();
        socketRef.current = null;
  ...
  const addMessage = useCallback(
    (text, receiverId) => {
      if (
        ...
        toast({
          title: "Error",
          description: "Not connected. Cannot send message.",
          variant: "destructive",
        });
        return;
      }
      ...
      const optimisticMessage = {
        ...
        avatar:
          currentApiUser.avatar ||
          `https://placehold.co/40x40.png?text=${
            currentApiUser.fullname && currentApiUser.fullname.length > 0
              ? currentApiUser.fullname[0].toUpperCase()
              : "U"
          }`,
        text,
      ...
      setUsersState((prev) =>
        prev.map((u) =>
          u.id === receiverId ? { ...u, lastMessageContent: text } : u
        )
      );

      console.log("AppContext: Emitting send_message", {
        receiver: receiverId,
        content: text,
        tempId,
      });
  ...
  const sendTypingIndicator = useCallback(
    (isTyping, toUserId) => {
      if (
        ...
        console.log(
          `AppContext: sendTypingIndicator - cannot send. Socket connected: ${!!(
    ...
    <AppContext.Provider
      value={{
        ...
        mapBackendMessageToFrontend: (
          ...
          mapBackendMessageToFrontendHelper(
            backendMsg,
            participatingUsers,
            currentUserId || (currentApiUser?.id ?? "")
          ),
